...
Like qlik/script_parser.py, this is deliberately NOT a full M language
interpreter -- M supports arbitrary function definitions, `each` lambdas,
custom connectors, and computed paths that a text-level parser cannot safely
evaluate. What this module resolves with confidence:
...
PARSER_LIMITATIONS = (
    ...
    "columns survive is not analyzed -- unlike the Qlik tool, this module "
    "does not attempt field-level (column-level) provenance, only "
    "table/entity-level source resolution."
)
    ...
    for func in _DIRECT_SOURCE_FUNCTIONS:
        ...
        if selector:
            ...
            if schema and item:
                ...
                resolved_table = item
        sources.append(
            DirectSource(connector=func, connection_args=args, resolved_table=resolved_table, selector=selector)
        )
    return sources
...
def split_shared_queries(document: str) -> dict[str, str]:
    ...
    M section -- see mashup_parser module docstring / README for the
    Common Data Model reference this is based on.
    """
    queries = {}
